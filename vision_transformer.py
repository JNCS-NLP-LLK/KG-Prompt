# ------------------------------------------
# Modified Code from Jaeho Lee, (https://github.com/JH-LEE-KR/dualprompt-pytorch)
# ------------------------------------------
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            use_e_prompt=False, e_prompt_layer_idx=None, use_prefix_tune_for_e_prompt=False, same_key_value=False,text_supervision=True, args=None, classes=None, clip_model=None,class_mask= None):
        """
        Args:
        ...
        if use_g_prompt and g_prompt_length is not None and len(g_prompt_layer_idx) != 0:
            # clip model
            self.clip_model, self.proprocess = clip.load("ViT-L/14", jit=False)
            # Task Level Language Guidance Prompts
            self.map_class={}
            self.task_prompts= []
            for j in range(len(class_mask)):
                self.map_class[j] = class_mask[j]
                class_name_start = self.class_names[class_mask[j][0]]
                text= "A photo of "+class_name_start.replace("_"," ")
                for i in range(1,len(class_mask[j])):
                    class_idx = class_mask[j][i]
                    class_name = self.class_names[class_idx]
                    text = text + " or " +class_name.replace("_"," ")
                self.task_prompts.append(text+".")
            self.task_prompts_tokens = clip.tokenize(self.task_prompts).cuda()
            with torch.no_grad():
                self.task_cls = self.clip_model.encode_text(self.task_prompts_tokens) 
            

            if self.args.subparser_name=="cifar100_lgcl":
                superclass=[]
                self.task_prompts2= []
                filename = 'hypernym_cifar100.txt'
                ...
                for line in lines:
                    line = line.split("to")
                    line=line[-1].strip()
                    superclass.append(line)

                for j in range(len(class_mask)):
                    text= "Classname belongs to "+superclass[len(class_mask[j])*j]
                    for i in range(1,len(class_mask[j])):
                        if superclass[len(class_mask[j])*j+i] not in text:
                           text = text + " or " +superclass[len(class_mask[j])*j+i]
                    self.task_prompts2.append(text+".")
                #print(self.task_prompts2)
                self.task_prompts2_tokens = clip.tokenize(self.task_prompts2).cuda()
                with torch.no_grad():
                    self.task_super = self.clip_model.encode_text(self.task_prompts2_tokens)
            

            elif self.args.subparser_name=="imr_lgcl":
                new_mask=[]
                for j in range(len(class_mask)):
                    for i in range(len(class_mask[0])):
                        new_mask.append(self.class_names[class_mask[j][i]].replace("_"," "))
                superclass={}#字典
                self.task_prompts2= []
                filename = 'hypernym_imr.txt'
                with open(filename, "r") as file1:
                    lines = file1.readlines()  # 读取文件中的所有行
                for line in lines:
                    line = line.strip().split(" belongs to ")
                    superclass[line[0]]=line[1]
                
                for j in range(len(class_mask)):
                    text= "Classname belongs to "+superclass[new_mask[j]]
                    for i in range(1,len(class_mask[j])):
                        if superclass[new_mask[len(class_mask[j])*j+i]] not in text:
                           text = text + " or " +superclass[new_mask[len(class_mask[j])*j+i]]
                    self.task_prompts2.append(text+".")
                #print(self.task_prompts2)
                self.task_prompts2_tokens = clip.tokenize(self.task_prompts2).cuda()
                with torch.no_grad():
                    self.task_super = self.clip_model.encode_text(self.task_prompts2_tokens)
            self.task_cls = self.task_cls*(args.tf_weight1)+self.task_super*(args.tf_weight2)  



            # Class Level Language Guidance Prompts
            self.clip_model.eval()
            prompts_natural= ["A photo of {}.".format(cl.replace("_"," ")) for cl in self.class_names]
            prompts_natural_tokens= clip.tokenize(prompts_natural).cuda()
            with torch.no_grad():
                self.natural_cls = self.clip_model.encode_text(prompts_natural_tokens) 
            self.natural_cls = self.natural_cls.cuda()
            ...
            if self.args.subparser_name=="cifar100_lgcl":
                ...
                for line in lines:
                    ...
                    line=line[-1].strip()
                    superclass.append(line)
                prompts_superclass= ["{} belongs to {}.".format(cl.replace("_"," "),superclass[i]) for i,cl in enumerate(self.class_names)]
                #print(prompts_superclass)
                prompts_superclass_tokens= clip.tokenize(prompts_superclass).cuda() 
                with torch.no_grad():
                    self.superclass_cls = self.clip_model.encode_text(prompts_superclass_tokens) 
            
            elif self.args.subparser_name=="imr_lgcl":
                prompts_superclass=[]
                for i in range(len(new_mask)):
                    prompts_superclass.append("{} belongs to {}".format(new_mask[i],superclass[new_mask[i]]))
                #print(prompts_superclass)
                prompts_superclass_tokens= clip.tokenize(prompts_superclass).cuda() 
                with torch.no_grad():
                    self.superclass_cls = self.clip_model.encode_text(prompts_superclass_tokens) 
                self.superclass_cls = self.superclass_cls.cuda()

           
            self.natural_cls =self.natural_cls *(args.cf_weight1)+self.superclass_cls *(args.cf_weight2)
       
            #print(self.natural_cls.shape)
        ...
        if use_e_prompt and e_prompt_layer_idx is not None:
            self.e_prompt = EPrompt(length=prompt_length, embed_dim=embed_dim, embedding_key=embedding_key, prompt_init=prompt_init,
                    ...
                    num_heads=num_heads, same_key_value=same_key_value,clip_model=self.clip_model, class_names=classes,natural_cls=self.task_cls, class_mask=class_mask, task_cls = self.task_cls)
    ...
    def forward_features(self, x, task_id=-1, cls_features=None, train=False):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            if self.use_g_prompt or self.use_e_prompt:
                if self.use_prompt_mask and train and self.use_e_prompt and self.e_prompt is not None:
                ...
                if self.use_e_prompt and self.e_prompt is not None:
                    res = self.e_prompt(x, prompt_mask=prompt_mask, cls_features=cls_features, task_id = task_id)
                ...
                for i, block in enumerate(self.blocks):
                    ...
                    elif i in self.e_prompt_layer_idx and self.use_e_prompt and self.e_prompt is not None:
                        e_prompt_counter += 1
    ...
    def forward_head(self, res, pre_logits: bool = False):
        ...
        res['pre_logits'] = x
        # normed features
        res['pre_normed'] = x/x.norm(dim=-1,keepdim=True)
        if self.use_g_prompt:

            res['natural_cls']= self.natural_cls.unsqueeze(0).tile((x.shape[0],1,1))
        x = self.fc_norm(x)
