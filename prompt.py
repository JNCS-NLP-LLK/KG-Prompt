    ...
    def forward(self, x_embed, prompt_mask=None, cls_features=None, task_id = -1 ):
        ...
        if self.prompt_pool:
            ...
            if self.use_prefix_tune_for_e_prompt:
                ...
                #print(idx,self.prompt.shape,batched_prompt_raw.shape)
            ...
            if task_id == 0:
                ...
                for other_task_id in range(task_id):
                    #print("other_task_id",other_task_id)
                    neg_rep_per_task = (batched_key_norm * task_reps[other_task_id].unsqueeze(0))  # bsz, 1, 768
                    #neg_rep_per_task = neg_rep_per_task.squeeze(1)  # bsz, 768
                    neg_rep_pool.append(neg_rep_per_task)

                neg_rep_cat = torch.cat(neg_rep_pool, dim=1)  # bsz, task_id-1, 768
                #print(neg_rep_cat.shape)

                # 对于每个样本，找出前k个最难负样本的得分
                k_hard_negatives = min(4, task_id)  # 取Top-k难负样本
                #print(k_hard_negatives)
                _, top_k_indices = neg_rep_cat.topk(k=k_hard_negatives, dim=1, largest=True)  # 获取top-k索引
                top_k_scores = torch.gather(neg_rep_cat, dim=1, index=top_k_indices)  # 根据索引获取top-k得分

                # 计算难负样本总和并归一化
                hard_neg_scaler_rep = torch.sum(top_k_scores.view(-1), dim=0) / (x_embed.shape[0] * k_hard_negatives)
                positive_scaler_rep = torch.sum(positive_rep)/x_embed.shape[0]

                # 更新总损失
                #print("hard_neg_scaler_rep",hard_neg_scaler_rep)
                total_loss = 1 - positive_scaler_rep + hard_neg_scaler_rep
