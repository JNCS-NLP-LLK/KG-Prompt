...
def build_continual_dataloader(args):
    ...
    for i in range(args.num_tasks):
        ...
        if args.distributed and utils.get_world_size() > 1:
            ...
            sampler_train_fixed = torch.utils.data.DistributedSampler(
            ...
            sampler_val = torch.utils.data.SequentialSampler(dataset_val)
            sampler_train_fixed = torch.utils.data.SequentialSampler(dataset_train)
        ...
        data_loader_train_fixed = torch.utils.data.DataLoader(
            dataset_train, sampler=sampler_train_fixed,
            batch_size=args.batch_size,
            num_workers=args.num_workers,
            pin_memory=args.pin_mem,
        )
        ...
        dataloader.append({'train': data_loader_train, 'val': data_loader_val,'train_fixed': data_loader_train_fixed})
